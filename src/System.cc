...
// 以下语句均在对ORB_SLAM2命名空间中的函数及变量进行定义
namespace ORB_SLAM2
{
//通过构造函数System::System()对SLAM系统进行初始化，传入ORB字典，配置文件，传感器类型等
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
               const bool bUseViewer):mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false),mbActivateLocalizationMode(false),
        mbDeactivateLocalizationMode(false) //冒号对参数进行初始化赋值
{
    ...
    //Check settings file
    cv::FileStorage fsSettings(strSettingsFile.c_str(), cv::FileStorage::READ);//调用函数fsSettings()，根据参数strSettingsFile读取配置文件,并由isOpened()判断文件是否存在
                                                                                //._str()是string类的一个函数，返回字符串的首地址
